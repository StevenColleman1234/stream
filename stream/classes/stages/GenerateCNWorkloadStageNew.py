...
class GenerateCNWorkloadStage(Stage):
    ...
    def __init__(self, list_of_callables, *, workload, accelerator,
        ...
        # elif CN_define_mode == "hardcoded1":
        #    self.factor_loops = hint_loops
        # else:
    ...
    def run(self):
        ...
        for node in nx.topological_sort(self.workload):
            if not isinstance(node,
                              ComputationNode):  # If other node types shouldn't be included in finer node graph, add here
                self.node_output_tensors, self.output_name_to_node = self.update_node_output_tensors_for_non_cn(node,
                                                                                                                self.node_output_tensors,
            ...
            tempzip = zip(node.input_names, node.variable_input_operands)
            for input_name, input_operand in tempzip:  # assume these two are aligned
                try:
        ...
        for node in G.nodes():
            ...
            for layer_operand in node.operand_list:
                ...
                if layer_operand in node.constant_operands:
                    tensor.base_priority = nb_nodes_per_layer_id[layer_id] - nb_seen_nodes_per_layer_id[layer_id]
                elif layer_operand == node.output_operand:
                    # Look at the amount of successors from different layers
                    tensor.base_priority = len([succ for succ in G.successors(node) if succ.id[0] != layer_id])
                else:
                    tensor.base_priority = 1  # This currently isn't used (Input tensors are not used)
    ...
    def update_node_output_tensors_for_non_cn(node, node_output_tensors, output_name_to_node):
        ...
        if isinstance(node, ReshapeNode):
            ...
            node_input_names = node.input_names
            assert len(node_input_names) == 1, "TransposeNode has more than one input."
            node_input_name = node.input_names[0]
            node_output_names = node.output_names
            assert len(node_output_names) == 1, "TransposeNode has more than one output."
            node_output_name = node_output_names[0]
            # Update the dict by adding an entry for the output based on the reshaped input
            input_tensor = node_output_tensors[node_input_name]
            output_tensor = node.transpose_operand_tensor(input_tensor)
            node_output_tensors[node_output_name] = output_tensor
            output_name_to_node[node_output_name] = node
        elif isinstance(node, LpNormalizationNode):
            node_input_names = node.input_names
            assert len(node_input_names) == 1
            node_input_name = node.input_names[0]
            node_output_names = node.output_names
            assert len(node_output_names) == 1
            node_output_name = node_output_names[0]
            # Update the dict by adding an entry for the output based on the reshaped input
            input_tensor = node_output_tensors[node_input_name]
    ...
    def get_outer_tmap_loop_dimensions(self, layer) -> List[TemporalLoop]:
                ...
                if loop_name in layer.loop_dim_size:
                    if loop_size == 'all' or layer.loop_dim_size[loop_name] < loop_size:
                        ...
                        try:
                            ...
                            new_loop_size = self.find_the_closest_divisible_factor_within_a_range(
        ...
        elif self.CN_define_mode == "hardcodedvalidation1":
            loop_name = 'B'
            outer_loops.append(TemporalLoop(loop_name, layer.loop_dim_size[loop_name]))

        elif self.CN_define_mode == "hardcodedLBL":
            loop_name = 'B'
            #outer_loops.append(TemporalLoop(loop_name, layer.loop_dim_size[loop_name]))

        elif self.CN_define_mode == "hardcodedC":
            loop_name = 'C'
            outer_loops.append(TemporalLoop(loop_name, int(np.ceil(layer.loop_dim_size[loop_name]/64))))

        elif self.CN_define_mode == "hardcodedB":
            loop_name = 'B'
            outer_loops.append(TemporalLoop(loop_name, int(np.ceil(layer.loop_dim_size[loop_name]/64))))

        elif self.CN_define_mode == "hardcodedK":
            loop_name = 'K'
            outer_loops.append(TemporalLoop(loop_name, int(np.ceil(layer.loop_dim_size[loop_name]/64))))

        else:
            for fixed_mapping in range(1,245):
                temp_mapping = "hardcoded" + str(fixed_mapping)
                if self.CN_define_mode == temp_mapping:
                    compute_mapping = fixed_mapping - 1
                    temp_mapping1 = compute_mapping % 3
                    compute_mapping = floor(compute_mapping / 3)
                    temp_mapping2 = compute_mapping % 3
                    compute_mapping = floor(compute_mapping / 3)
                    temp_mapping3 = compute_mapping % 3
                    compute_mapping = floor(compute_mapping / 3)
                    temp_mapping4 = compute_mapping % 3
                    compute_mapping = floor(compute_mapping / 3)
                    temp_mapping5 = compute_mapping % 3
                    id = layer.id[0]
                    if id == 0:
                        if temp_mapping1 == 0:
                            loop_name = 'K'
                        ...
                        raise ValueError("Hardcoded example only works for two MatMul layers with Transpose in between")

            if self.accelerator.name == 'Big_single_core_b_c_k':
                if loop_name == 'C':
                    outer_loops.append(TemporalLoop('C', int(np.ceil(layer.loop_dim_size['C']/16))))
                if loop_name == 'B':
                    outer_loops.append(TemporalLoop('B', int(np.ceil(layer.loop_dim_size['B']/16))))
                if loop_name == 'K':
                    outer_loops.append(TemporalLoop('K', int(np.ceil(layer.loop_dim_size['K']/16))))
            if self.accelerator.name == 'Big_single_core_b_c':
                if loop_name == 'C':
                    outer_loops.append(TemporalLoop('C', int(np.ceil(layer.loop_dim_size['C']/64))))
                if loop_name == 'B':
                    outer_loops.append(TemporalLoop('B', int(np.ceil(layer.loop_dim_size['B']/64))))
                if loop_name == 'K':
                    outer_loops.append(TemporalLoop('K', int(np.ceil(layer.loop_dim_size['K']/64))))
            if self.accelerator.name == 'Big_single_core_b_k':
                if loop_name == 'C':
                    outer_loops.append(TemporalLoop('C', int(np.ceil(layer.loop_dim_size['C'] / 64))))
                if loop_name == 'B':
                    outer_loops.append(TemporalLoop('B', int(np.ceil(layer.loop_dim_size['B'] / 64))))
                if loop_name == 'K':
                    outer_loops.append(TemporalLoop('K', int(np.ceil(layer.loop_dim_size['K'] / 64))))
            if self.accelerator.name == 'Big_single_core_c_k':
                if loop_name == 'C':
                    outer_loops.append(TemporalLoop('C', int(np.ceil(layer.loop_dim_size['C'] / 64))))
                if loop_name == 'B':
                    outer_loops.append(TemporalLoop('B', int(np.ceil(layer.loop_dim_size['B'] / 64))))
        ...
        for op in variable_operands:
            ...
            if op == node.output_operand:
                ...
                precision = node.operand_precision[op] * (
                    not is_source_node)  # if this layer is the first layer, we assume the inputs are streamed and "free"
            ...
            for finer_node, should_add_to_tensor in zip(finer_nodes_list, should_add_to_tensor_list):
                ...
                window = tuple([slice(max(0, start), stop) for (start, stop) in
                                op_dim_ranges])  # start can be negative for padding which, makes np flip
                ...
                bounded_op_dim_ranges = [range(max(0, start), min(max_stop, stop)) for ((start, stop), max_stop) in
                                         zip(op_dim_ranges, op_dim_ranges_max_stop)]
    ...
    def get_finer_nodes(original_node, outer_temporal_loops) -> List[ComputationNode]:
        ...
        for i, outer_loop in enumerate(outer_temporal_loops):
            ...
            outer_span = prod([temporal_loop.size for temporal_loop in lower_outer_cn_loops if
                               temporal_loop.dimension == loop_dim])  # Returns 1 if empty list
        ...
        for n in range(nb_cns):
            ...
            finer_node = ComputationNode(finer_node_id, finer_node_attrs_copy, finer_node_name, finer_node_input_names,
                                         finer_node_output_names, produces_final_output)
    ...
    def get_intra_edges(nodes):
        # temp = len(nodes) - 1
        # for node_id in range(temp):
        #    nodes[node_id]
        return [(nodes[node_id], nodes[node_id + 1], {'size': 0, 'bits': 0}) for node_id in range(len(nodes) - 1)]
        # return [(nodes[node_id], nodes[node_id + 1], {'size': 0, 'bits': 0}) for node_id in range(len(nodes))]
    ...
    def get_inter_edges_tensor_based(producer_output_tensor, consumer_input_tensor):
        ...
        """
        assert producer_output_tensor.shape == consumer_input_tensor.shape, f"Arrays to construct inter-layer edges must be equal shape {producer_output_tensor.shape}, {consumer_input_tensor.shape}."
        inter_edges = set()
