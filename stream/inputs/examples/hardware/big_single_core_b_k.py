...
from inputs.examples.hardware import architecture_definition as arch_def
from inputs.examples.hardware import architecture_description as arch_des
...
#
def get_dataflows():
    return [{'D1': ('B', 64), 'D2': ('K', 64)}]

multiplier_array = arch_des.multiplier_array_2D()
memory_hierarchy = arch_des.memory_hierarchy_B_K_dataflow(multiplier_array)
...
cores_graph = get_2d_mesh([core_1], 1, 1, 256, 0, simd_core=simd_core, offchip_core=offchip_core)

global_buffer = None
accelerator = Accelerator("Big_single_core_b_k", cores_graph, global_buffer, offchip_core_id=offchip_core_id)